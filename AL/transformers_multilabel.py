...
def get_optimizer(model: nn.Module, lr: float, epochs: int, tr_steps: int,
                  wrm_steps: int):
...
class BertCollator:
    ...
    def __call__(self, batch):
        ...
        text, labels = zip(
            *[[sample['text'], sample['labels']] for sample in batch])
...
def get_datasets(train_data_path: str,
                 val_data_path: str, test_data_path: str, text_column: str):
...
def load_ckp(checkpoint_fpath: str, model: nn.Module, optimizer: torch.optim)\
        -> Dict:
    """Load a saved model.
...
def save_ckp(state: Dict, is_best: bool, checkpoint_path: str,
...
def find_best_cut(preds, targets) -> Dict:
    ...
    thrl = [0.00001, 0.0001, 0.001, 0.01] + \
...
def train_model(n_epochs: int, training_loader: DataLoader,
    ...
    for epoch in range(n_epochs):
        ...
        val_outputs = []
        model.train()
        print('############# Epoch {}: Training Start   #############'.format(
            epoch))
        ...
        print('############# Epoch {}: Training End     #############'.format(
            epoch))

        print('############# Epoch {}: Validation Start   #############'.format(
            epoch))
        ######################
        # validate the model #
        ######################
...
def main(cfg: DictConfig) -> None:
    ...
    datasets = get_datasets(train_data_path, val_data_path, test_data_path,
                            'comment_text')
    # set parameters
    train_params = {'batch_size': cfg.experiment.batch_size,
                    'shuffle': True,
                    'num_workers': cfg.experiment.num_workers
                    }
    test_params = {'batch_size': cfg.experiment.batch_size,
                   'shuffle': False,
                   'num_workers': cfg.experiment.num_workers}

    training_loader = DataLoader(datasets['train'],
                                 collate_fn=BertCollator(cfg.experiment.tok),
                                 **train_params)
    validation_loader = DataLoader(datasets['val'],
                                   collate_fn=BertCollator(cfg.experiment.tok),
                                   **test_params)
    testing_loader = DataLoader(datasets['test'],
                                collate_fn=BertCollator(cfg.experiment.tok),
                                **test_params)
    model = TransformersTextClassification(cfg.experiment.model,
    ...
    opt_sch = get_optimizer(model=model, lr=cfg.experiment.learning_rate,
    ...
    if cfg.experiment.test_only:
        ...
        sys.exit(0)
    trained_model = train_model(n_epochs=cfg.experiment.epochs,
